...
export default class InstrumentEditor {
  ...
  redrawCurve() {
    ...
    if (env_vol) {
      ...
      for (let i = 0; i < len; i += 2) {
        const pos = this.curvePointPosition(i);
      ...
      if (this.curveX != null) {
        ...
        const pos = this.canvasPositionFromCurvePoint(this.curveX, val);
  ...
  canvasPositionFromCurvePoint(curvex, curvey) {
    const height = this.canvas.height;
  ...
  curvePointAtPosition(x, y) {
    const len = this.instrument.env_vol.points.length;

    for (let i = 0; i < len; i += 2) {
      const p = this.curvePointPosition(i);
  ...
  curvePositionFromCoordinates(xpos, ypos) {
